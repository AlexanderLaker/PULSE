...
// ═══════════════════════════════════════════════════════════════
// PULSE Profit Pool Shift Model — Real V12 Data (5,000 Bayesian MC Iterations)
// ═══════════════════════════════════════════════════════════════
...
const DAG_EDGES = [{"from":"Government","to":"Technology","weight":0.6},{"from":"Government","to":"Customer","weight":0.4},{"from":"Government","to":"Environmental","weight":0.3},{"from":"Consumer","to":"Customer","weight":0.5},{"from":"Consumer","to":"Competitive","weight":0.4},{"from":"Consumer","to":"Technology","weight":0.3},{"from":"Technology","to":"Consumer","weight":0.4},{"from":"Technology","to":"Competitive","weight":0.5},{"from":"Technology","to":"Customer","weight":0.3},{"from":"Environmental","to":"Government","weight":0.6},{"from":"Environmental","to":"Consumer","weight":0.4},{"from":"Environmental","to":"Technology","weight":0.3},{"from":"Customer","to":"Competitive","weight":0.5},{"from":"Customer","to":"Consumer","weight":0.3},{"from":"Competitive","to":"Customer","weight":0.4},{"from":"Competitive","to":"Consumer","weight":0.3}];
...
function DAG() {
  ...
  const pos={Consumer:{x:300,y:55},Customer:{x:520,y:135},Technology:{x:520,y:290},Competitive:{x:300,y:370},Environmental:{x:80,y:290},Government:{x:80,y:135}};
  return <div style={G}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",...H}}><span>Causal DAG — 16 Directed Edges</span>{sel&&<span style={{fontSize:11,color:"#818cf8",textTransform:"none",letterSpacing:0}}>Shock: {sel}</span>}</div>
    <svg viewBox="0 0 600 430" style={{width:"100%",maxHeight:360}}>
      {DAG_EDGES.map((e,i)=>{const p1=pos[e.from],p2=pos[e.to];if(!p1||!p2)return null;const dx=p2.x-p1.x,dy=p2.y-p1.y,len=Math.sqrt(dx*dx+dy*dy),nx=dx/len,ny=dy/len;const a=sel===e.from;return<line key={i} x1={p1.x+nx*40} y1={p1.y+ny*40} x2={p2.x-nx*40} y2={p2.y-ny*40} stroke={a?"#818cf8":"rgba(255,255,255,0.08)"} strokeWidth={a?2.5:1} opacity={a?1:0.4}/>;
      })}
      {Object.entries(pos).map(([f,p])=><g key={f} onClick={()=>setSel(sel===f?null:f)} style={{cursor:"pointer"}}><circle cx={p.x} cy={p.y} r={36} fill={sel===f?FC[f]:"rgba(26,26,46,0.8)"} stroke={FC[f]} strokeWidth={sel===f?2.5:1.5}/><text x={p.x} y={p.y-4} textAnchor="middle" dominantBaseline="middle" style={{fontSize:16}}>{FI[f]}</text><text x={p.x} y={p.y+16} textAnchor="middle" dominantBaseline="middle" fill={sel===f?"#fff":"#ccc"} style={{fontSize:8,fontWeight:700,letterSpacing:0.5}}>{f.toUpperCase()}</text></g>)}
    </svg>
    {!sel&&<div style={{textAlign:"center",fontSize:11,color:"#55556a"}}>Click a force to highlight causal edges</div>}
  </div>;
}
  ...
  return <div style={{minHeight:"100vh",background:"#0a0a0f",color:"#f0f0f5",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display','Inter',sans-serif"}}>
    ...
    <div style={{position:"sticky",top:0,zIndex:40,backdropFilter:"blur(20px)",background:"rgba(10,10,15,0.8)",borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"12px 24px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div style={{display:"flex",alignItems:"center",gap:16}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontSize:14,fontWeight:700,letterSpacing:"-0.3px"}}>PULSE</div><div style={{fontSize:8,color:"#55556a",textTransform:"uppercase",letterSpacing:2}}>Profit Pool Shift Model</div></div>
        </div>
    ...
    <div style={{maxWidth:1440,margin:"0 auto",padding:24,display:"flex",flexDirection:"column",gap:20}}>
      {tab==="overview" && <>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}><DAG/><FRadar/></div>
      </>}
      {tab==="trends" && <TrendExplorer onSelectCategory={setSelCat}/>}
      <div style={{textAlign:"center",padding:16,fontSize:10,color:"#55556a"}}>PULSE v2.1 \u00b7 Bayesian MC t-Copula (\u03BD=4) \u00b7 Causal DAG (16 edges) \u00b7 Financial Firewall active \u00b7 All values relative % only</div>
    </div>
  </div>;
}
