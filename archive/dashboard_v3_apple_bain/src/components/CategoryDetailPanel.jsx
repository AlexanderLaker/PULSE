...
/**
 * MiniPathChart: Area chart showing shift path with confidence bands
 */
const MiniPathChart = ({ pathData }) => {
  ...
  const yAxisDomain = [-maxAbs * 1.1, maxAbs * 1.1];
...
/**
 * ForceDecomposition: Horizontal bars showing each force's contribution
 */
const ForceDecomposition = ({ decomposition }) => {
  ...
  const forces = Object.keys(decomposition)
    .filter(f => decomposition[f] !== 0)
    .sort((a, b) => Math.abs(decomposition[b]) - Math.abs(decomposition[a]));
...
/**
 * ContributingTrendItem: Trend entry in the list
 */
const ContributingTrendItem = ({ trend, index }) => {
...
/**
 * CategoryDetailPanel — Main component
 */
export default function CategoryDetailPanel({ data, categoryId, onClose }) {
  const category = useMemo(() => {
    if (!categoryId || !data) return null;
    // Find category from data or CATEGORIES
  ...
  const forceDecomposition = useMemo(() => {
    if (!data?.force_decomposition?.[categoryId]) return {};
    return data.force_decomposition[categoryId];
  }, [data, categoryId]);

  const trendList = useMemo(() => {
    if (!data?.contributing_trends?.[categoryId]) return [];
    return data.contributing_trends[categoryId].sort((a, b) => Math.abs(b.score) - Math.abs(a.score));
  }, [data, categoryId]);
