#!/usr/bin/env python3
"""
v3.12 attenuation calibration: recalibration on the 51-driver base after the
Toilet Care split (release 2.12.0, owner ruling O13, September 2026).

Identical method and identical mechanism adjustments to v3.11. The ONLY input
that changed is the exposure space: the category vectors are now 13 long
because Toilet Care was split out of Hard-Surface Cleaner, so the pairwise
Jaccard overlaps and the random-pair baseline J0 move slightly and the derived
attenuation moves with them. Re-running is not optional: the calibration is a
function of the exposure space, so leaving v3.11 in place after a taxonomy
change would silently mislabel the provenance (the F-28 failure mode).

Same method as v3.1/v3.5 (docs/Attenuation_Calibration_Methodology.md):
  Step 1: pairwise weighted Jaccard on the 13-category exposure vectors
...
Outputs
  ...
  data/attenuation_calibration_v3_12.json  machine-readable, tracked; the
                                           source of the 2.12.0 defaults in
...
CALIBRATION_VERSION = "calibrated_v3.12_september2026"
V3_5_JSON = REPO / "data" / "attenuation_calibration_v3_5.json"
OUT_JSON = REPO / "data" / "attenuation_calibration_v3_12.json"
...
def main(out_json: Path = OUT_JSON, verbose: bool = True):
    ...
    say("\n=== Per-force effective attenuation (v3.12 | v3.5 | delta) ===")
    for f in FORCES:
    ...
    out = {
        ...
        "base": "51-driver core set, September 2026 review, 13-category taxonomy (release 2.12.0, owner ruling O13; taxonomy per O10/O11 base)",
        "method": "v3.1 methodology: weighted Jaccard on 13-category exposure vectors "
                  "(the O13 taxonomy: Toilet Care split out of Hard-Surface Cleaner), excess over J0, "
