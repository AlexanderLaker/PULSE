...
Inputs (both in the repository so the base is reproducible):
  data/trend_base_2026-09/core_set_51_v3.json   the 51 reviewed drivers
...
CORE_SET = ROOT / "data" / "trend_base_2026-09" / "core_set_51_v3.json"
...
def render_seed(recs: list[dict], verdicts: dict) -> str:
    ...
    a("data/trend_base_2026-09/core_set_51_v3.json (the reviewed core set of the")
    ...
    a("CAT_KEYS = list(CATEGORIES)  # 12 categories in order")
    a("VC_KEYS = list(VC_STEPS)     # 8 VC steps in order")
    a("REG_KEYS = list(REGIONS)     # 4 regions in order")
    a("")
    a("def cat(color, care, styling, body, fcn, fca, ffi, lad, hdw, adw, hsc, ic):")
    a("    return dict(zip(CAT_KEYS, [color, care, styling, body, fcn, fca, ffi, lad, hdw, adw, hsc, ic]))")
    a("")
