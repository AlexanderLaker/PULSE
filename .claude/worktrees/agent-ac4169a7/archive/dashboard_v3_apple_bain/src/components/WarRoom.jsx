...
import {
  ...
  Brain, AlertTriangle, FileDown, Settings, X, RefreshCw,
} from 'lucide-react';
...
function generateMockData() {
  ...
  categoryIds.forEach(catId => {
    ...
    forceContributions[catId] = forceNames.map(force => ({
      force,
      value: Math.random() / total,
      normalized: 0,
    }));
    const sum = forceContributions[catId].reduce((s, x) => s + x.value, 0);
    forceContributions[catId] = forceContributions[catId].map(fc => ({
      ...fc,
      normalized: fc.value / sum,
    }));
  });
  ...
  const allocation = categoryIds.map((catId) => {
...
export default function WarRoom() {
  ...
  // AI insights mock
  const aiInsights = [
    { id: 1, type: 'signal', text: '3 new signals detected', count: 3 },
    { id: 2, type: 'trigger', text: 'FCN trigger breached', severity: 'warning' },
  ];
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: T.text2, letterSpacing: 0.5 }}>
              PULSE War Room
            </div>
            <div
              style={{
            ...
            />
            <span style={{ fontSize: 9, fontWeight: 600, color: T.text3, textTransform: 'uppercase', letterSpacing: 0.8 }}>
              v3.0
            </span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginLeft: 'auto' }}>
            ...
            <div style={{ ...WarRoomStyles.pill, background: T.border1 }}>
