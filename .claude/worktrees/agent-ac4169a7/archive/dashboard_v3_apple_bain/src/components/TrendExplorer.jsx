...
/**
 * DotBar: Interactive 1-5 dot selector. Editable: click a dot to set score.
 * Hover effects, color by semantic meaning (blue=impact, amber=probability, emerald=exposure).
 */
const DotBar = ({ value, onChange, editable = false, color = 'blue', size = 'sm' }) => {
...
/**
 * CategoryExposureGrid: Edit category exposure scores for a trend.
 * Shows 12 categories (Hair + LHC) grouped with editable 0-5 dot scores.
 */
const CategoryExposureGrid = ({ exposures, onChange }) => {
  const grouped = {
    'Hair': CATEGORIES.filter(c => c.group === 'Hair'),
    'LHC': CATEGORIES.filter(c => c.group === 'LHC'),
  };

  const handleChange = (catId, newVal) => {
...
/**
 * ValueChainExposureGrid: Edit value chain step exposure scores (Raw Materials through After-Sales).
 */
const ValueChainExposureGrid = ({ exposures, onChange }) => {
  const vcSteps = [
  ...
  const handleChange = (stepId, newVal) => {
...
/**
 * ExpandedTrendRow: Detail view showing all editable fields + description/implication.
 */
const ExpandedTrendRow = ({ trend, onUpdateTrend, onClose }) => {
  const [catExposure, setCatExposure] = useState(trend.category_exposure || {});
  const [vcExposure, setVcExposure] = useState(trend.vc_exposure || {});
        ...
        >
          ...
          <div style={{
            ...
            <button
              ...
              onMouseEnter={(e) => {
                e.target.style.backgroundColor = T.bg4;
                e.target.style.borderColor = T.border2;
              }}
              onMouseLeave={(e) => {
                e.target.style.backgroundColor = T.bg3;
...
/**
 * TrendExplorer — Main component
 * Props: { data, forceFilter, onForceFilter, onUpdateTrend }
 * data.trends = [{ id, force, name, direction, impact, probability, score, gp1_shift, ... }]
 */
export default function TrendExplorer({ data, forceFilter, onForceFilter, onUpdateTrend }) {
      ...
      <div style={{
        ...
        {forces.map((force) => (
          <button
            ...
            style={{
              padding: force === 'All' ? '6px 12px' : '6px 12px',
              borderRadius: '16px',
              fontSize: '11px',
              fontWeight: 500,
              transition: 'all 120ms ease',
              backgroundColor: forceFilter === force
                ? force === 'All' ? T.accent : FORCE_COLORS[force]
                : T.bg3,
          ...
          >
            {force === 'All' ? '◆ All Forces' : `${FORCE_ICONS[force]} ${force}`}
          </button>
        ))}
      </div>
