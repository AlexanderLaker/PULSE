...
class TestGoldenPins:
    ...
    Engine: bayesian_copula MODEL_VERSION 2.5.0. Regenerate pins ONLY for
    deliberate model changes, in the same commit as the change.
    """

    PINS = {
        # cat:               (median,            p10,                p90)
        "Hair: Color": (-0.003444376207, -0.005674497196, -0.000926332174),
        "Hair: Care":  (-0.003444376207, -0.005674497196, -0.000926332174),
        "LHC: FCN":    (-0.006405246146, -0.008246549127, -0.004066599240),
    }

    def test_engine_identity(self, mock_model_config, mock_trends_database):
        r = _run(mock_model_config, mock_trends_database)
        assert r["model_version"] == "2.5.0"
