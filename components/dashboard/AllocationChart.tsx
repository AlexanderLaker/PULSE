 ...
 *   shifts_2030?: Record<string, number>
 * }
 *
 * Displays: sorted by weight descending, with delta from current allocation,
 * plus expected return, portfolio risk metrics.
 */

import { useMemo, FC } from 'react';
import { motion } from 'framer-motion';
import type { AllocationRecommendation, CategoryId } from '@/types';
import { T, CATEGORIES, shortCat } from '@/lib/format';
...
const AllocationChart: FC<AllocationChartProps> = ({ allocation = null }) => {
  // Parse allocation data
  const items = useMemo(() => {
    if (!allocation) return [];

    let result: AllocationItem[] = [];

    if (allocation.weights && typeof allocation.weights === 'object') {
