...
Database persistence: Uses pulse.database (Postgres on Vercel, SQLite locally).
"""
...
class DelphiProtocol:
    ...
    All sessions, rounds, and calibrations are persisted to the database
    (Postgres on Vercel, SQLite locally).
    """

    def __init__(self):
        self.rounds: list[ScoringRound] = []
        self.calibration_exercises: list[CalibrationExercise] = []
        self.scorer_calibration: dict = {}
        self.scorer_bias_profiles: dict = {}
        self.sessions: Dict[str, DelphiSession] = {}
        ...
        try:
            with get_db_connection() as conn:
                ...
                for raw_row in cursor.fetchall():
                    ...
                    comp_at = row.get("completed_at")
                    if isinstance(comp_at, str):
                        comp_at = datetime.fromisoformat(comp_at)
                    ...
                    session = DelphiSession(
                        ...
                        trend_ids=json.loads(row["trend_ids"]) if row.get("trend_ids") else [],
                        scorer_ids=json.loads(row["scorer_ids"]) if row.get("scorer_ids") else [],
                    )
    ...
    def start_round(self, session_id: str, round_number: int, trend_ids: List[str],
        ...
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f"UPDATE delphi_sessions SET current_round = {p}, status = {p} WHERE id = {p}",
    ...
    def inter_rater_reliability(self, trend_id: str,
        ...
        """Compute Krippendorff's alpha (simplified via coefficient of variation)."""
        relevant = [r for r in self.rounds if r.trend_id == trend_id]
        if round_number:
    ...
    def calibration_exercise(self, scorer_id: str, known_outcomes: Dict[str, tuple]) -> Dict[str, Any]:
        ...
        if not self.calibration_exercises:
            return {
                "calibration_factor": 1.0, "biases": [],
                "mean_impact_error": 0.0, "mean_prob_error": 0.0,
        ...
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(f"DELETE FROM delphi_calibration WHERE scorer_id = {p}", (scorer_id,))
                cursor.execute(
                    f"""
    ...
    def create_session(
        ...
        session = DelphiSession(
            ...
            trend_ids=trend_ids or [], scorer_ids=scorer_ids or [],
        )
        ...
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute(
                    ...
                    """,
                    (
                        session_id, name, description, session.status,
                        session.current_round, json.dumps(trend_ids or []),
    ...
    def get_scorer_history(self, scorer_id: str) -> Dict[str, Any]:
        ...
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute(
                    f"SELECT * FROM delphi_calibration WHERE scorer_id = {p}",
                    (scorer_id,),
                )
                calibration_row = cursor.fetchone()
                calibration = None
                if calibration_row:
                    cr = _row_to_dict(calibration_row)
                    calibration = {
    ...
    def advance_session_round(self, session_id: str) -> Dict[str, Any]:
        """Advance session to next round and return anonymized distribution."""
        if session_id not in self.sessions:
            return {"error": "Session not found"}
    ...
    def complete_session(self, session_id: str) -> Dict[str, Any]:
        """Finalize session, compute consensus scores."""
        if session_id not in self.sessions:
            return {"error": "Session not found"}
        ...
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f"UPDATE delphi_sessions SET status = {p}, completed_at = {p} WHERE id = {p}",
    ...
    def apply_consensus_to_trends(self, session_id: str, trend_db: Any = None) -> Dict[str, Any]:
        ...
        p = placeholder()
        try:
            from pulse.database import log_audit, get_trend_by_id as db_get_trend
    ...
    def _ensure_tables_exist(self):
        """Ensure Delphi-specific database tables exist.

        Tables are now created by init_db() in database.py.
        This method is kept for backward compatibility but just calls init_db().
        """
        try:
            init_db()
            logger.info("Delphi database tables ensured via init_db()")
        except Exception as e:
            logger.debug(f"Delphi tables init: {e}")
