...
# step anywhere. Same method as v3.1/v3.5 (weighted Jaccard on the 12-category
...
# Generated by scripts/compute_attenuation_v3_11.py; the JSON
# data/attenuation_calibration_v3_11.json is the record and the test lock.
#
# Provenance per force (v3.11 — delta vs v3.5 in parens):
#   force            cross-force row mean   eff_att   (Δ v3.5)
#   Consumer                  0.026        0.487     (-0.008)
#   Customer                  0.165        0.418     (+0.017)
#   Technology                0.170        0.415     (-0.019)
#   Government                0.108        0.446     (+0.031)
#   Environmental             0.243        0.379     (-0.039)
#   Competitive               0.041        0.480     (+0.001)
#
# Trend-weighted mean across the 51 drivers = 0.4520 (v3.5: 0.4523).
# J0 random-pair baseline = 0.4265 over 1,275 pairs (v3.5: 0.4525 over 4,851).
# Source: data/attenuation_calibration_v3_11.json; workbook data/Attenuation_Calibration_v3_11.xlsx.
...
DEFAULT_ATTENUATION_SOURCE = "calibrated_v3.11_september2026"  # valid sources: "calibrated_v3.11_september2026" | "calibrated_v3.5_april2026" (legacy) | "calibrated_v3.1_april2026" (legacy) | "admin_override"
...
# the 48 composite cells (12 categories × 4 regions) are rolled up to the
# category and portfolio numbers with the relevant part of the HCB P&L that
# actually sits in each category × region combination — a 12 × 4 matrix of
...
# tests/test_bayesian_mc.py). Until the actual figures are loaded every cell
# counts 1/48 — equal regions AND equal categories (owner decision). HCB's
...
DEFAULT_CELL_WEIGHTS_SOURCE = (
    "Equal placeholder: each of the 48 category x region cells = 1/48 of the "
    "HCB gross-profit pool (owner decision 2026-09-03/10). Replace with the "
    "actual HCB gross-profit share per cell via run_50k_prod.py --cell-weights "
    "FILE when the P&L data are loaded."
)
...
def cell_weights_outer(region_weights: dict, category_weights: dict) -> dict:
    """Build a separable 12 × 4 cell-weight matrix from two marginal vectors.
...
DEFAULT_FORCE_OVERLAP_MATRIX = {
    # ── CALIBRATED on the 51-driver base (September 2026, v3.11; owner ruling O11) ──
    # Methodology (unchanged since v3.1): excess overlap above the random-pair
    # baseline + force-size asymmetry + per-cell mechanism adjustment.
    #
    # Step 1 (empirical): mean pairwise weighted Jaccard between the drivers of
    #   every cross-force pair over the 12-category exposure vectors, converted
    #   to excess overlap max(0, mean_J − J₀) / (1 − J₀) with J₀ = 0.426.
    # Step 2 (asymmetry): × sqrt(n_j / n_i) capped at 1.5 (a narrow force is
    #   more likely covered by a broad one).
    # Step 3 (mechanism): additive ±0.03–0.10 per cell, RE-JUDGED for the 51
    #   drivers (owner decision 2026-09-10). Non-zero cells and their reasons
    #   (symmetric unless stated; full text in the calibration JSON):
    #   Consumer ↔ Customer: +0.05
    #   Consumer ↔ Government: +0.03
    #   Consumer ↔ Competitive: +0.05
    #   Customer ↔ Technology: +0.08
    #   Customer ↔ Government: +0.03
    #   Customer ↔ Environmental: +0.03
    #   Customer ↔ Competitive: +0.08
    #   Technology ↔ Government: +0.05
    #   Technology ↔ Environmental: +0.05
    #   Technology ↔ Competitive: +0.03
    #   Government ↔ Environmental: +0.05
    #   Deliberately 0: Consumer–Environmental (C-06 and E-12 share the 2026
    #   energy shock on different P&L lines: demand vs COGS — co-movement is the
    #   copula's job, not overlap), Consumer–Technology, Government–Competitive,
    #   Environmental–Competitive.
    #
    # Rows = 'how much of row_force is covered by col_force'. Row means give
    # DEFAULT_PER_FORCE_ATTENUATION above (identity eff_att = 0.5 × (1 − mean)).
    # Pre-2.11 note: the v3.5 numbers that stood here were the v3.1 cell values
    # with hand edits, not the v3.5 JSON (see FINDINGS_REGISTER F-28); since
    # 2.11.0 all three layers come from one generated JSON.
    "Consumer":       {"Consumer": 0.000, "Customer": 0.051, "Technology": 0.000, "Government": 0.030, "Environmental": 0.000, "Competitive": 0.050},
    "Customer":       {"Consumer": 0.052, "Customer": 0.000, "Technology": 0.239, "Government": 0.208, "Environmental": 0.220, "Competitive": 0.104},
    "Technology":     {"Consumer": 0.000, "Customer": 0.266, "Technology": 0.000, "Government": 0.223, "Environmental": 0.333, "Competitive": 0.030},
    "Government":     {"Consumer": 0.030, "Customer": 0.155, "Technology": 0.154, "Government": 0.000, "Environmental": 0.199, "Competitive": 0.000},
    "Environmental":  {"Consumer": 0.000, "Customer": 0.363, "Technology": 0.450, "Government": 0.402, "Environmental": 0.000, "Competitive": 0.000},
    "Competitive":    {"Consumer": 0.050, "Customer": 0.123, "Technology": 0.030, "Government": 0.000, "Environmental": 0.000, "Competitive": 0.000},
}
...
DEFAULT_WITHIN_FORCE_OVERLAP = {
    # ── CALIBRATED on the 51-driver base (September 2026, v3.11; owner ruling O11) ──
    # Methodology: mean pairwise weighted Jaccard over the 12-category exposure
    # vectors WITHIN each force, converted to excess overlap above the
    # random-pair baseline J₀ = 0.426, plus a mechanism adjustment
    # re-judged per force for the 51 drivers; clamped to [0.10, 0.45].
    # Consumed by the engine's n_eff dampening (F2): 1 − ov·(n_eff − 1)/n_eff.
    #
    # Empirical signal per force (raw mean J → excess → mech → final | v3.5):
    #   Consumer       0.358 → 0.000 +0.00 → 0.100 | 0.100
    #   Customer       0.525 → 0.173 +0.05 → 0.223 | 0.242
    #   Technology     0.612 → 0.323 +0.05 → 0.373 | 0.206
    #   Government     0.470 → 0.076 +0.05 → 0.126 | 0.312
    #   Environmental  0.621 → 0.339 +0.05 → 0.389 | 0.314
    #   Competitive    0.288 → 0.000 +0.00 → 0.100 | 0.100
    #
    # Reasons (full text in data/attenuation_calibration_v3_11.json):
    #   Consumer: 20 drivers, one mechanism each after the review (duplicated
    #     regional lines, GLP-1 shedding, longevity claims consolidated);
    #     residual premium-hair (C-03/C-04/C-07/C-02), value
    #     (C-01/C-06/C-11/C-35) and demographic (C-05/C-25/C-18/C-34) clusters
    #     act on different P&L lines or regions; empirical excess 0, floor
    #     0.10 applies, no adjustment
    #   Customer: K-01 discounters, K-12 drugstores, K-03 buyer power and K-13
    #     retail media all compress the same European trade margin on the same
    #     LHC volume; kept separate because the channels differ, but the P&L
    #     line is one
    #   Technology: T-11 AI agents, T-13 generative search and T-08 platform
    #     replenishment are one digital-intermediation cluster (three of six
    #     drivers); the high raw overlap also reflects category-broad vectors,
    #     so the adjustment stays at the v3.5 level
    #   Government: six of ten drivers come out of one legislative programme
    #     (microplastics, PPWR/EPR, EmpCo, EUDR/CSDDD, Detergents Regulation,
    #     UWWTD) and G-04, G-16 and G-17 stack compliance cost on the same
    #     detergent SKUs; the review removed the triple count of the packaging
    #     stack, the cluster remains
    #   Environmental: four COGS-side cost drivers on the same European
    #     manufactured volume; E-07 energy, E-12 oil feedstock and E-03 carbon
    #     move with the same 2026 energy shock
    #   Competitive: four drivers with distinct actors and mechanisms (scale
    #     leaders, challengers, share windows, salon channel); empirical
    #     excess 0, floor 0.10 applies, no adjustment
    #
    # Pre-2.11 note: the values that stood here since v3.2 were the v3.1 numbers
    # (0.100/0.157/0.232/0.426/0.269/0.100) labelled v3.5; the v3.5 JSON held
    # 0.100/0.242/0.206/0.312/0.314/0.100 and was never transferred (F-28).
    "Consumer":       0.100,
    "Customer":       0.223,
    "Technology":     0.373,
    "Government":     0.126,
...
# unchanged; scripts/compute_attenuation_v3_11.py records the value
...
class ModelConfig:
    ...
    # share}}, 12 × 4, sum 1. region_weights and category_weights are DERIVED
    ...
    def __post_init__(self):
        ...
        Three constructions are accepted:
          ...
          • nothing given → DEFAULT_CELL_WEIGHTS (equal 1/48).
